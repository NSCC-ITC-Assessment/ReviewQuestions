...
All assessment files are written under the `_assessment/` folder in the repository. Any directory component of the `output_file` setting is ignored — only the basename is used.
...
- Instructors can see assessments for all branches at a glance in the `_assessment/` folder
