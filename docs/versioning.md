...
2. The workflow updates the `image:` line in `action.yml` to the immutable patch tag (e.g. `docker://ghcr.io/nscc-itc-assessment/grillmycode:v1.0.3`)
...
gh api orgs/NSCC-ITC-Assessment/packages/container/GrillMyCode/versions --jq '.[].metadata.container.tags'
```
